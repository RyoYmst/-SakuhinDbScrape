...
<<<<<<< HEAD
	...
	for url,image in zip(all_url,image_url)[5:6]:
		each_comic_data = {}
...
=======
	...
	for url,image in zip(all_url,image_url):
		each_comic_data = {}
		html = urllib2.urlopen(url)
		soup = BeautifulSoup(html)
		title = GetTitle(soup)
...
def Output(data,reviews):
	for i,each_comic_data in zip(range(0,len(data)),data):
		try:
			write_data = codecs.open("../masterthesis_review/"+ data[i]["title"] +".txt","w","utf-8")
			write_data.write(each_comic_data["review_num"]) 
			for each_comic_review in reviews:
				write_data.write(each_comic_review)
			write_data.close()
		except IOError:
			print "例外検出"
def main():
	...
	link_url = []
	image_url = []
	url = "http://sakuhindb.com/anime/j_manga_popular_301.html"
	html = urllib2.urlopen(url)
	soup = BeautifulSoup(html)
...
>>>>>>> 2771635efce87b7c182c3d07e5189da9df5cec45
